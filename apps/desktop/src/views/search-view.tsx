...
export function SearchView() {
  ...
  const results = useQuery({ queryKey: ["project-search", query], queryFn: () => searchProject(query), enabled: query.trim().length > 0 });
  return <section className="search-view">
    ...
    <div className="search-toolbar"><label className="search-field"><Search size={15} /><input aria-label="搜索项目内容" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="输入关键词" /></label><button type="button" className="secondary-action" onClick={() => void rebuildSearchIndex().then(() => client.invalidateQueries({ queryKey: ["project-search"] }))}><RefreshCw size={15} />重建索引</button></div>
    <div className="search-results">{results.isPending && query.trim() ? <p className="plan-empty">正在搜索…</p> : null}{results.isError ? <p className="project-error" role="alert">搜索失败：{String(results.error)}</p> : null}{!results.isPending && query.trim() && results.data?.length === 0 ? <p className="plan-empty"><Database size={16} />没有匹配结果。</p> : null}{results.data?.map((item) => <article className="search-result" key={`${item.objectType}-${item.objectId}`}><div><span className="entity-type-badge">{item.objectType}</span><code>{item.sourceVersion ?? "无来源版本"}</code></div><p>{item.snippet}</p></article>)}</div>
  </section>;
}
