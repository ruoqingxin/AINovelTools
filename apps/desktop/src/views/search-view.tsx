...
export function SearchView() {
  ...
  return <section className="search-view">
    ...
    <div className="search-results">{results.isPending && query.trim() ? <p className="plan-empty">正在搜索…</p> : null}{results.isError ? <p className="project-error" role="alert">搜索失败：{errorMessage(results.error)}</p> : null}{!results.isPending && query.trim() && results.data?.length === 0 && offset === 0 ? <p className="plan-empty"><Database size={16} />没有匹配结果。</p> : null}{results.data?.map((item) => <article className="search-result" key={`${item.objectType}-${item.objectId}`}><div><span className="entity-type-badge">{item.objectType}</span><code>{item.sourceVersion ?? "无来源版本"}{item.blockId ? ` · 块 ${item.blockId}` : ""}</code></div><p>{item.snippet}</p></article>)}{results.data && results.data.length === 50 ? <button type="button" className="secondary-action search-more" onClick={() => setOffset((value) => value + 50)}>加载更多</button> : null}</div>
  </section>;
}
