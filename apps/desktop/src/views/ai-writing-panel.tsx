...
import { Ban, Check, Columns2, LoaderCircle, Play, Sparkles, ThumbsDown, ThumbsUp, Trash2 } from "lucide-react";
...
export function AiWritingPanel(props: { chapterId: string; chapterTitle: string; chapterPlan: string; draft: string; editor: Editor | null }) {
  ...
  return <section className="ai-panel" aria-label="AI 创作">
    ...
    <AiModelNote taskLabel="正文书写" profile={selectedChatProfile} preference={selectedChatPreference} />
    ...
    {pending.length ? <div className="proposal-list">
      ...
      {pending.map(({ proposal, validation, feedback }) => <article className="proposal" key={proposal.id}>
