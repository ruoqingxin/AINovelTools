...
import { Ban, Check, KeyRound, LoaderCircle, Play, Save, Sparkles, Trash2 } from "lucide-react";
...
export function AiWritingPanel(props: { chapterId: string; chapterTitle: string; chapterPlan: string; draft: string; editor: Editor | null }) {
  ...
  async function removeSecret() {
    ...
    setError(null);
    try {
      await deleteModelSecret(profileId);
      await client.invalidateQueries({ queryKey: ["model-profiles"] });
    } catch (cause) { setError(errorMessage(cause)); }
  }

  async function runAction(action: AiAction) {
    if (!profileId || !props.editor) return;
  ...
  const selectedProfile = profiles.data?.find((item) => item.id === profileId);
  ...
  return <section className="ai-panel" aria-label="AI 创作">
    ...
    <details className="model-settings" open={!profiles.data?.length}>
      ...
      {profiles.data?.length ? <select value={profileId} onChange={(event) => setProfileId(event.target.value)} aria-label="模型配置">
      ...
      <div className="model-grid">
        ...
        <label>云端服务<select value={form.provider} onChange={(event) => { const provider = event.target.value as ModelProfileInput["provider"]; setForm({ ...form, provider, ...(provider === "SILICON_FLOW" ? { baseUrl: "https://api.siliconflow.cn/v1" } : {}) }); }}><option value="SILICON_FLOW">硅基流动</option><option value="OPEN_AI_COMPATIBLE">通用 OpenAI-compatible</option></select></label>
