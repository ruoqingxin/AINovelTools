...
export function ProjectWorkspaceView(props: { mode?: "planning" | "chapters" | "writing" } = {}) {
  ...
  const [chapterTab, setChapterTab] = useState<ChapterWorkspaceTab>(workspaceMode === "chapters" ? "plan" : "editor");
  ...
  function selectNode(node: PlanNode) {
    ...
    setChapterTab(workspaceMode === "chapters" ? "plan" : "editor");
    return true;
  }
  ...
  return (
    <section className="project-workspace">
      <div className="workspace-heading">
        ...
        <p className="workspace-lede">{workspaceMode === "chapters" ? "集中维护章节执行卡，使用 AI 生成内容，并处理修订冲突和草稿恢复。正文页只负责安静写作。" : workspaceMode === "writing" ? "选择章节后直接进入编辑器。章节执行卡、AI 创作和修订管理请在章节页处理。" : "作品设定、故事大纲和分卷管理按顺序推进，章节和场景把计划变成可执行的写作任务，正文区负责完成文学表达。"}</p>
      </div>
      ...
      {!selected ? <main className={`planning-dashboard${isChapterMode ? " writing-mode" : ""}`} aria-label={workspaceMode === "chapters" ? "章节工作台总览" : workspaceMode === "writing" ? "正文工作区总览" : "项目规划总览"}>
          ...
          <div className="planning-overview-heading"><div><span className="planning-kicker">{workspaceMode === "chapters" ? "章节工作台" : workspaceMode === "writing" ? "正文工作区" : "建议下一步"}</span><h2>{isChapterMode ? (chapterNodes.length ? workspaceMode === "chapters" ? "选择章节开始准备" : "选择章节开始写作" : "还没有章节") : planningHeadline}</h2><p>{isChapterMode ? (chapterNodes.length ? workspaceMode === "chapters" ? "执行卡、AI 创作、修订冲突和草稿恢复集中在这里。" : "正文页保持简洁，只提供章节选择与正文编辑。" : "请先在规划页创建章节，再进入后续工作。") : "按当前流程完成作品设定后，将依次开放故事大纲、分卷管理和章节规划。"}</p></div>{isChapterMode ? chapterNodes[0] ? <button type="button" className="primary-action" onClick={() => selectNode(chapterNodes[0])}><ArrowRight size={15} />打开第1章</button> : <a className="primary-action" href="/planning"><ArrowRight size={15} />前往规划页</a> : <button type="button" className="primary-action" onClick={() => void continuePlanning()}><ArrowRight size={15} />继续下一步</button>}</div>
        ...
        {chapterFocusHeader}
        {selected.kind === "CHAPTER" && workspaceMode === "chapters" ? <div className="chapter-workspace-navigation"><ChapterWorkspaceTabs value={chapterTab} onChange={setChapterTab} recoveryCount={recovery.data?.length ?? 0} /></div> : null}
        ...
        {selected.kind === "CHAPTER" && workspaceMode === "planning" ? <div className="planning-redirect-panel"><BookOpen size={18} /><div><strong>章节已拆分，进入章节工作台继续准备</strong><span>执行卡、AI 创作、修订冲突和草稿恢复统一放在章节页；正文页只负责专注写作。</span></div><a href={`/chapters#${selected.id}`}>准备本章</a></div> : null}
        {selected.kind === "CHAPTER" && isChapterMode ? <div className="chapter-editor">
          {workspaceMode === "writing" ? <div className="chapter-tab-panel focused-writing-panel" id="chapter-panel-editor" role="tabpanel">
            {editor ? <>
          ...
          {workspaceMode === "chapters" && chapterTab === "ai" ? <div className="chapter-tab-panel" id="chapter-panel-ai" role="tabpanel" aria-labelledby="chapter-tab-ai"><AiWritingPanel chapterId={selected.id} chapterTitle={selected.title} chapterPlan={nodePlanDraft} volumeId={selectedVolume?.id ?? ""} volumePlan={selectedVolumePlan?.content ?? ""} draft={draft} editor={editor} /><div className="chapter-ai-savebar" data-dirty={chapterDirty || undefined}><div><strong>{chapterDirty ? "AI 内容已应用到正文草稿" : "正文草稿没有待保存修改"}</strong><span>{chapterDirty ? "先保存正文修订，再前往正文页继续打磨。" : "接受 AI 候选后，可在这里直接保存。"}</span></div><button type="button" className="primary-action" onClick={() => void saveDraft()} disabled={savingDraft || !chapterDirty}>{savingDraft ? "保存中…" : "保存正文修订"}</button><a className="secondary-action" href={`/writing#${selected.id}`}>打开正文</a></div></div> : null}
          {workspaceMode === "chapters" && chapterTab === "revisions" ? <div className="chapter-tab-panel" id="chapter-panel-revisions" role="tabpanel" aria-labelledby="chapter-tab-revisions">
          ...
          {workspaceMode === "chapters" && chapterTab === "recovery" ? <div className="chapter-tab-panel" id="chapter-panel-recovery" role="tabpanel" aria-labelledby="chapter-tab-recovery">{recovery.data?.length ? <div className="recovery-banner"><span>发现 {recovery.data.length} 条可恢复草稿</span><button type="button" className="secondary-action" onClick={() => void recoverLatest()}>恢复最近草稿</button></div> : <div className="plan-empty">当前没有可恢复的草稿。</div>}</div> : null}
        </div> : null}
      </main>}
      </div> : null}
    </section>
