  ...
  return (
    <section className="project-workspace">
      <div className="workspace-heading">
        ...
        <p className="workspace-lede">{workspaceMode === "chapters" ? "按顺序完成章节执行卡、创作准备和一致性审核，再生成可以进入正文的候选。" : workspaceMode === "writing" ? "正文页承接已经生成或手写的内容，负责编辑保存、版本恢复和成稿后的知识提取。" : "作品设定、故事大纲和分卷管理按顺序推进，章节和场景把计划变成可执行的写作任务，正文区负责完成文学表达。"}</p>
      </div>
      ...
      {!selected ? <main className={`planning-dashboard${isChapterMode ? " writing-mode" : ""}`} aria-label={workspaceMode === "chapters" ? "章节工作台总览" : workspaceMode === "writing" ? "正文工作区总览" : "项目规划总览"}>
          ...
          <div className="planning-overview-heading"><div><span className="planning-kicker">{workspaceMode === "chapters" ? "章节工作台" : workspaceMode === "writing" ? "正文工作区" : "建议下一步"}</span><h2>{isChapterMode ? (chapterNodes.length ? workspaceMode === "chapters" ? "选择章节开始准备" : "选择章节开始写作" : "还没有章节") : planningHeadline}</h2><p>{isChapterMode ? (chapterNodes.length ? workspaceMode === "chapters" ? "从执行卡开始，依次准备、审核并生成正文候选。" : "编辑并保存正文后，再处理版本恢复和知识提取。" : "请先在规划页创建章节，再进入后续工作。") : "按当前流程完成作品设定后，将依次开放故事大纲、分卷管理和章节规划。"}</p></div>{isChapterMode ? chapterNodes[0] ? <button type="button" className="primary-action" onClick={() => selectNode(chapterNodes[0])}><ArrowRight size={15} />打开第1章</button> : <a className="primary-action" href="/planning"><ArrowRight size={15} />前往规划页</a> : <button type="button" className="primary-action" onClick={() => void continuePlanning()}><ArrowRight size={15} />继续下一步</button>}</div>
        ...
        {selected.kind === "CHAPTER" && workspaceMode === "planning" ? <div className="planning-redirect-panel"><BookOpen size={18} /><div><strong>章节已拆分，进入章节工作台继续准备</strong><span>先完成执行卡、创作准备和一致性审核，再生成正文候选；正文定稿后的版本与知识提取在正文页处理。</span></div><a href={`/chapters#${selected.id}`}>准备本章</a></div> : null}
