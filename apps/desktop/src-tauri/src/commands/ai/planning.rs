...
pub(crate) fn incomplete_generation_failure(
...
pub(crate) fn persist_ai_run_request(
    state: &ProjectState,
    run_id: uuid::Uuid,
...
pub(crate) fn planning_output_looks_truncated(output: &str) -> bool {
    ...
    match result {
        Ok(outcome) => {
            ...
            if completion.is_complete() {
                let _ = manager.complete_ai_run(run_id, usage.as_ref(), output_tokens);
