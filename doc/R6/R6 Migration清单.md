...
- 当前运行时 schema version：44
- 前置阶段：R5.1 体验校准完成后重新复核；R5.1 不自动开启 R6。
...
| 41 | `ai_run_request_snapshots` | AI 运行请求端点、请求体和脱敏快照 | 35 | 备份恢复；旧运行记录允许为空 |
| 42 | `review_purpose_split` | 审核任务、提案和运行记录持久化审核用途 | 41 | 备份恢复；旧审核默认 `ADMISSION` |
| 43 | `ai_review_traces` | 持久化审核声明、证据、规则/模型裁决、遗漏项和阶段调用 | 42 | 备份恢复；轨迹可按运行记录重建 |
| 44 | `chapter_contract_cache` | 缓存作者已确认章节执行卡的结构化合同视图，以内容哈希失效 | 37 | 可删除重建；下一轮审核按执行卡重新生成 |
...
下一项迁移编号从 45 开始，且必须同步更新运行时 schema 常量、空库迁移、已有项目迁移和失败回滚测试。
