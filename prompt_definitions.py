...
知识库设定与用户内容指导是创作硬约束。不得擅自替换其中已有的专有名词、人物身份、力量规则和世界历史；如两者冲突，以用户内容指导为准。
...
必须优先沿用知识库已有角色、身份、关系和专有名词。仅在知识库确实没有覆盖时才能补充新角色，不得用新设定覆盖已有设定。
...
必须整理和沿用知识库已有的历史、地域、势力、力量体系及术语，不得另造一套与其冲突的世界观。空缺处才可合理补充，并保持兼容。
...
情节必须建立在知识库既有设定上，并实际使用其中相关的人物、地点、规则、术语和伏笔。不得把知识库内容仅作为装饰，也不得生成与之冲突的替代设定。
...
# =============== 5. 分卷规划与章节蓝图 ===================
volume_plan_prompt = """\
请根据以下小说架构，为一部长篇小说设计分卷规划。

全书总章节数：{number_of_chapters}
计划分卷数：{volume_count}（最多20卷）
小说架构：
{novel_architecture}

请为每一卷给出：卷号、卷名、覆盖章节范围、核心目标、主要冲突、卷末推进，以及需要延续到后卷的悬念。
各卷必须是中期阶段收束，不能把非最后一卷写成全书大结局。
只返回分卷规划正文，不解释过程，不使用 Markdown 代码块。
"""
...
这是一本总计{number_of_chapters}章的长篇小说。本次只生成第{n}章到第{m}章，
本次范围不是全书结局；除非第{m}章等于总章数，否则严禁写“大结局”、最终决战或收束全书。

【局部蓝图防剧透规则】
- 小说架构中出现的最终真相、终局方案、最终敌人、世界晋升原理和历史全貌，只能作为长期规划参考。
- 本次只推进当前范围内的局部目标，不得提前确认上述终局答案，不得一次性解释完整世界历史或反派计划。
- 前期章节最多确认一个与当前事件直接相关的新事实；其余重要信息必须以线索、矛盾记录或不完整证词保留。
- 每章结尾必须留下可延展的下一阶段目标，不能让主线、核心谜团和最终冲突在本次范围内同时闭合。

【当前进度披露上限】
{stage_guardrail}

【设定一致性】
- 人名、势力、境界、遗藏等级和专有名词必须来自小说架构；禁止自行创造近似名称或不存在的等级。
- 低阶遗藏只能表现其等级允许的记忆与意识强度，不能让高阶完整人格在低阶遗藏中直接现身。
- 作者层面的伏笔名称也应使用当前阶段可见的中性描述，避免后续章节生成时把终局答案直接写给读者。
- 伏笔名称使用“未知共鸣、异常痕迹、不完整记录”等中性表述，不得用终局答案给未知线索命名。
- 逐章核对伤亡人数、持有物、人物位置、已知信息和时间顺序；后续章节不得与前章已经发生的事实矛盾。
- 地理移动必须有交通、时间与中转过程支撑；短章节范围内不得突然跨越大域或直接抵达远方核心区域。
- 开局和早期范围的章末钩子应与当前能力层级匹配，禁止用高阶遗藏、世界级警报或全局势力集体关注强行升级。
...
- 使用精炼语言描述，每章字数控制在100字以内。
- 合理安排节奏，确保整体悬念曲线的连贯性。
- 本次只输出第{n}章至第{m}章，章节编号必须连续且不得超出范围。
- 只有当本次范围覆盖全书最后一章时才允许安排全书结局；局部范围必须留下可延展的阶段目标、冲突和悬念。
- 不得把架构中的终局设定改写成当前章节的确定事实；需要披露时，只披露当前卷应知道的局部版本。
...
这是当前记录的剧情要点与未解决冲突（可为空）：
{old_plot_arcs}

请更新这份剧情追踪文档。仅保留仍会影响后续章节的内容，并按以下分类输出：
# 未解决冲突
# 待回收伏笔
# 角色承诺与目标
# 时间线约束
...
要求：
- 已在本章解决的事项应删除或标记为已解决后移除
- 每条内容必须来自现有文本，不得推测尚未发生的事实
- 使用简短条目，避免复述章节正文
- 某个分类没有内容时写“无”

仅返回更新后的剧情追踪文档，不要解释任何内容。
"""
...
你是小说策划编辑。请根据用户意见，重新编写一份完整的小说架构。

基础参数：
- 主题：{topic}
- 类型：{genre}
- 计划章节数：{number_of_chapters}
- 每章目标字数：{word_number}
...
改写规则：
1. 当前架构不为空时，以其为基础保留未被用户意见否定的有效设定；
2. 当前架构为空时，根据基础参数和用户意见从头构建；
3. 输出必须是完整、可直接替换原文件的小说架构，包含小说设定、核心种子、角色动力学、世界观和情节架构；
4. 保持人物目标、世界规则和主线冲突相互一致；
5. 只返回改写后的完整小说架构，不解释修改过程，不使用 Markdown 代码块。
"""

architecture_section_revision_prompt = """\
你是小说策划编辑。请只修改当前小说架构中用户指定的一个分区。
...
完整小说架构（仅用于核对上下文和一致性，不得整体输出）：
{current_architecture}

改写规则：
1. 只返回“所选分区”的完整替换内容，第一行必须原样保留所选分区标题；
2. 可以调整该分区内部的子标题和内容，但不得输出所选分区以外的其他同级或上级分区；
3. 严格落实用户意见，并与完整架构中的人物、世界规则、时间线和主线保持一致；
4. 未被用户意见否定的有效设定应继续保留；
...
改写规则：
1. 当前蓝图不为空时，保留未被用户意见否定的有效章节安排；
2. 当前蓝图为空时，根据小说架构和用户意见从第 1 章开始重新规划；
3. 输出覆盖第 1 章至第 {number_of_chapters} 章的完整蓝图，不得只输出发生变化的章节；
4. 每章必须包含章节编号、标题、核心事件、角色推进、冲突或悬念，以及与前后章节的承接；
5. 保证章节编号连续，剧情节奏与小说架构一致；
6. 只返回改写后的完整章节蓝图，不解释修改过程，不使用 Markdown 代码块。
"""
