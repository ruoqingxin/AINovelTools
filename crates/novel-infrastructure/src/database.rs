...
impl Database {
    ...
    fn migrate(&self) -> Result<(), DatabaseError> {
        ...
        if applied.unwrap_or(0) < 29 {
            self.connection.execute_batch(
                "ALTER TABLE jobs RENAME TO jobs_before_ai;
                CREATE TABLE jobs (
                    id TEXT PRIMARY KEY NOT NULL,
                    job_type TEXT NOT NULL CHECK(job_type IN ('BACKUP','RESTORE_VERIFY','HEALTH_SCAN','REBUILD_SEARCH_INDEX','AI_PLANNING_GENERATE','AI_PLANNING_EXTRACT')),
                    payload TEXT NOT NULL DEFAULT '{}' CHECK(json_valid(payload)),
                    status TEXT NOT NULL CHECK(status IN ('QUEUED','RUNNING','SUCCEEDED','FAILED','CANCELLED')),
                    progress INTEGER NOT NULL DEFAULT 0 CHECK(progress BETWEEN 0 AND 100),
                    attempt_count INTEGER NOT NULL DEFAULT 0,
                ...
                INSERT INTO jobs SELECT * FROM jobs_before_ai;
                DROP TABLE jobs_before_ai;
            ...
            self.connection.execute_batch(
                ...
                CREATE TABLE jobs (
                    ...
                    job_type TEXT NOT NULL CHECK(job_type IN (
                        ...
                        'REFRESH_CHAPTER_SUMMARY','REFRESH_PROJECT_SETTING_SUMMARY',
                        'AI_PLANNING_GENERATE','AI_PLANNING_EXTRACT'
                    )),
                    payload TEXT NOT NULL DEFAULT '{}' CHECK(json_valid(payload)),
                    status TEXT NOT NULL CHECK(status IN ('QUEUED','RUNNING','SUCCEEDED','FAILED','CANCELLED')),
                    progress INTEGER NOT NULL DEFAULT 0 CHECK(progress BETWEEN 0 AND 100),
        ...
        if applied.unwrap_or(0) < 47 {
            self.connection.execute_batch(
                "PRAGMA foreign_keys=OFF;
                BEGIN;
                ALTER TABLE job_events RENAME TO job_events_before_setting_summary_refresh;
                ALTER TABLE jobs RENAME TO jobs_before_setting_summary_refresh;
                CREATE TABLE jobs (
                    id TEXT PRIMARY KEY NOT NULL,
                    job_type TEXT NOT NULL CHECK(job_type IN (
                        'BACKUP','RESTORE_VERIFY','HEALTH_SCAN','REBUILD_SEARCH_INDEX',
                ...
                SELECT id, job_type, payload, status, progress, attempt_count, cancel_requested,
                       error_summary, created_at, updated_at, acknowledged_at
                FROM jobs_before_setting_summary_refresh;
                CREATE TABLE job_events (
                    id TEXT PRIMARY KEY NOT NULL,
                    job_id TEXT NOT NULL REFERENCES jobs(id) ON DELETE CASCADE,
                    stage TEXT NOT NULL,
                ...
                INSERT INTO job_events (id, job_id, stage, message, progress, created_at)
                SELECT id, job_id, stage, message, progress, created_at
                FROM job_events_before_setting_summary_refresh;
                DROP TABLE job_events_before_setting_summary_refresh;
                DROP TABLE jobs_before_setting_summary_refresh;
                CREATE INDEX idx_jobs_status_updated ON jobs(status, updated_at);
                CREATE INDEX idx_job_events_job_created ON job_events(job_id, created_at);
                INSERT INTO schema_migrations (version, name)
                    VALUES (47, 'project_setting_summary_refresh_jobs');
                COMMIT;
                PRAGMA foreign_keys=ON;",
            )?;
        }
